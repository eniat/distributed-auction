...
public class ReplicaImpl extends UnicastRemoteObject implements ReplicatedAuction {
    ...
    public OperationResult handleClientOperation(Operation op, List<String> memberList) throws RemoteException {
        ...
        // Propose the operation to the rest of the replicas in the memberList (i.e., call propose remote method on members *excluding self* and ignore any unreachable replicas)
        int ackCount = 1;
        ...
        // If majority of replicas acknowledges (assume leader acks), then:
        // If a majority quorum is not achieved (or in case of other errors), return OperationResult.fail("") and provide a description of the error in the fail() method
        if (ackCount < majority(memberList.size())){
            return OperationResult.fail("Not enough acks");
        }
           //Locally execute the operation on self (as the leader) - you may use apply(), set the operation in the log as committed, and update any other state variables, if needed
        OperationResult result = apply(op);
        LogEntry logEntry = log.get(seqNo);
        logEntry.committed = true;
        lastCommitted = seqNo;
        lastApplied = seqNo;
           // Call commitUpTo on all replicas, again ignoring any unreachable replicas) 
        for (String replicaName : memberList) {
            if (replicaName.equals(this.myName)) {
                continue; // skip self
            }
    ...
    public boolean commitUpTo(long seqNo) {
        ...
        // Commit the local log entries upto seqNo if there are no missing log entries
        if (seqNo <= lastCommitted) {
            return true; // up to date
        }
        try{
            long nextToCommit = lastCommitted + 1;
            // If there are missing entries before seqNo in the local log, pull them from the leader (using getEntriesAfter)
            ...
            // Execute and commit the new operation(s) - you may use apply() on each operation
